...
"""
Copyright (c) 2015 Hervé Audren

This is a simple program to compute the stability polygon of a legged robot over n contacts.
It computes a set of extremum points via cone-programming and builds an outer and inner polygon from them.
See the paper by Bretl and Lall, "Testing static equilibrium for legged robots" (IEEE Transaction on Robotics, August 2008) for more details.

MIT license
https://github.com/haudren/shapes
...
if test:
  ...
  proc = PolygonInterpolator(p1s, p2s)
  poly = proc.interpolate(0.9)

  plt.plot(p1s.exterior.xy[0], p1s.exterior.xy[1], c='r')
  plt.plot(p2s.exterior.xy[0], p2s.exterior.xy[1], c='b')
  plt.plot(poly.exterior.xy[0], poly.exterior.xy[1], c='g')
  plt.show()
